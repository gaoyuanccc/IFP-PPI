def bin_interaction_transiform_gene( genome_length,interaction_matrix_filename, resolution_fp,gene_location_np1,gene_name_np,gene_gene_interaction_name_np,
                                     now_path_np,now_seq_np,bin_begin_number,input_folder):
    """
将bin之间的互作转换为基因间的互作
    ...
    for b1 in gene_location_np1.keys():  # 首先遍历基因的名字
        b2 = (int(gene_location_np1[b1][0]) // resolution_fp) + bin_begin_number  # 基因的初始位置除于1000，整数部分就是基因的初始位置所在的bin序号
        b3 = (int(gene_location_np1[b1][1]) // resolution_fp) + bin_begin_number  # 基因的末尾位置除于1000，整数部分就是基因的末尾位置所在的bin序号
        b4 = []  # 通过添加每个基因相对应的序号列表
...
def bin_interaction_transiform_gene_poly( genome_length, resolution_fp,gene_location_np1,gene_name_np,bin_begin_number):
    ...
    for gene1_name in gene_name_np:  # 获得每个基因的性质和bin序号列表的循环，得到基因的性质(是一个bin还是多个bin，字典格式 gene: 1或2)和得到基因和其相对应的bin序号列表（字典格式 gene : bin序号列表)
        ...
        if len(gene_bin[gene1_name]) == 1:  # 判断基因是否只位于一个bin内
            ...
            if len(gene_bin[gene1_name]) == 2:  # 判断基因是否只位于两个bin内
                o = float(gene_location_np1[gene1_name][1]) - float(bin_location[gene_bin[gene1_name][1]][0])  # o是基因所位于的第二个bin的部分的长度，是由基因的末尾位置减去基因所位于的第二个bin的初始位置
                o1 = gene_length[gene1_name] / o  # o1是基因的长度对o的倍数，是由基因的总长度比上o
                if o1 == 2:  # 判断基因是不是被两个bin平分了
                    for a6 in gene_bin[gene1_name]:
                        bin_number1.append(a6)
                ...
                if gene_location_np1[gene1_name][0] != bin_location[gene_bin[gene1_name][0]][0]:  # 基因的初始位置和第一个bin的初始位置不相同
                    if gene_location_np1[gene1_name][1] != bin_location[gene_bin[gene1_name][-1]][1]:  # 判断基因的末尾位置和最后一个bin的末尾位置不相同
                        for a1 in gene_bin[gene1_name]:  # 如果基因的初始位置和末尾位置都不相同，宾序号就是bin列表去除首尾
                            bin_number1.append(a1)
                        bin_number1.pop(0)
                        bin_number1.pop()
...
def bin_interaction_transiform_gene_interaction( gene_n,gene1_bin,interaction_matrix_filename,gene_gene_interaction_name_np,now_path_np,now_seq_np,input_folder):
    """
将bin之间的互作转换为基因间的互作
    :param gene_location_np1: 基因的位置信息的字典
    :param gene_name_np: 基因的名字的形参
    ...
    return gene_interaction_decimal
